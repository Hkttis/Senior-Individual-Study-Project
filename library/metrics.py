...
def stress_function(data,dni,pos_matrix) : 
    ...
    for row in data :
        ind1 = dni[row[0]]
        ind2 = dni[row[1]]
        distance = math.sqrt((pos_matrix[ind2][0]-pos_matrix[ind1][0])**2 + (pos_matrix[ind2][1]-pos_matrix[ind1][1])**2) / km2pix
        stress += (distance-int(row[2])/km2Li)**2
    return stress
