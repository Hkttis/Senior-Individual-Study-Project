"""Run all checks required before the formal anchor-split robustness experiment."""
...
from run_paper_script.ch5_anchor_split_robustness import (
    _parse_seed_list,
    preflight_anchor_split_robustness,
)
...
def main() -> int:
    ...
    try:
        report = preflight_anchor_split_robustness(
        ...
        print(f"[FAIL] Anchor robustness preflight failed: {exc}")
        return 1

    print("[OK] Anchor robustness preflight passed")
    print(f"  Selected splits: {report['n_selected_splits']}/{report['n_all_valid_splits']}")
    ...
    print(f"  HPO runs per split: {report['hpo_runs_per_split']}")
    print(f"  Final runs per split: {report['final_runs_per_split']}")
    print(f"  Expected total model runs: {report['expected_total_model_runs']}")
    print(f"  Existing completed splits: {report['existing_completed_splits']}")
    print(f"  Existing incomplete splits: {report['existing_incomplete_splits']}")
    print(f"  Free disk: {report['free_disk_bytes'] / 1_000_000_000:.2f} GB")
    print("  Weight mapping: w_dir proportional to 10^alpha; w_rep proportional to 10^beta")
    print(f"  Boundary policy: {report['boundary_policy']}")
